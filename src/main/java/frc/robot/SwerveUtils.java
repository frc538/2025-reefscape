...
public class SwerveUtils {
  ...
  public static double stepTowardsCircular(double current, double target, double stepSize) {
    ...
    if (difference <= stepSize) return target;
  ...
  public static double wrapAngle(double angle) {
    ...
    if (angle == twoPi) return 0;
    if (angle > twoPi) return angle - twoPi * Math.floor(angle / twoPi);
    if (angle < 0) return angle + twoPi * (Math.floor((-angle) / twoPi) + 1);
    return angle;
  }
